...
export abstract class BaseResult<T, E extends { kind: number }> {
   ...
   */
  static all<T, E extends { kind: number }>(results: Result<T, E>[]): Result<T[], E>;
  /**
  ...
  static all<T, E extends { kind: number }>(
    ...
    if (results.length > 0 && results[0] instanceof Promise) {
      return Promise.all(results as Promise<Result<T, E>>[]).then((resolved) =>
        BaseResult.all<T, E>(resolved),
      );
    }
    const values: T[] = [];
   ...
   */
  static any<T, E extends { kind: number }>(results: Result<T, E>[]): Result<T, E>;
  /**
   ...
   */
  orElse<U, F extends { kind: number }>(fn: (error: E) => Result<U, F>): Result<U, F>;
  ...
  map<U>(fn: (value: T) => U | Promise<U>): Result<U, E> | Promise<Result<U, E>> {
    if (this.success) {
   ...
   */
  mapErr<F extends { kind: number }>(fn: (error: E) => Promise<F>): Promise<Result<T, F>>;
